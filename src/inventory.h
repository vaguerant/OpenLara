...
struct OptionItem {
    ...
    float drawParam(float x, float y, float w, StringID oStr, bool active, uint8 value) const {
        if (oStr != STR_EMPTY) {
            UI::textOut(vec2(x + DPI(32), y), oStr);
            x = x + w * 0.5f;
            w = w * 0.5f - 32.0f;
        }
    ...
    float drawBar(float x, float y, float w, bool active, uint8 value) const {
        UI::renderBar(CTEX_WHITE_SPRITE, vec2(x + (DPI(32) + 2.0f), y - LINE_HEIGHT + 6 + 2), vec2(w - (64.0f + 4.0f), LINE_HEIGHT - 6 - 4), value / float(maxValue), color, 0xFF000000, 0xFFA0A0A0, 0xFFA0A0A0, 0xFF000000);
        UI::specOut(vec2(x + DPI(16), y), icon);
        if (active) {
...
*/
    ...
    void renderPassport(Item *item) {
        ...
        UI::textOut(vec2(eye, DPI(480 - 32)), str, UI::aCenter, UI::width);
        int tw = UI::getTextSize(STR[str]).x;

        if (item->value > 0) UI::specOut(vec2((UI::width - tw) * 0.5f - 32.0f + eye, DPI(480 - 32)), 108);
        if (item->value < 2) UI::specOut(vec2((UI::width + tw) * 0.5f + 16.0f + eye, DPI(480 - 32)), 109);
    ...
    void renderItemText(Item *item) {
        ...
        renderItemCount(item, vec2(UI::width / 2 - DPI(160), DPI(480 - 96)), DPI(320));
    ...
    // show health bar in inventory when selector is over medikit
        ...
        if (phaseChoose == 1.0f) {
            switch (item->type) {
                ...
                case TR::Entity::INV_STOPWATCH :
                case TR::Entity::INV_MAP       :
                    UI::textOut(vec2(0, DPI(240)), STR_EMPTY, UI::aCenter, UI::width);
                    break;
                default : ;
            }
        }
    ...
    void renderUI() {
        ...
        if (page == PAGE_SAVEGAME) {
            UI::renderBar(CTEX_OPTION, vec2(UI::width / 2 - 120, DPI(240) - 14), vec2(240, LINE_HEIGHT - 6), 1.0f, 0x802288FF, 0, 0, 0);
            UI::textOut(vec2(0, DPI(240)), pageTitle[page], UI::aCenter, UI::width);
            UI::renderBar(CTEX_OPTION, vec2(slot + UI::width / 2 - 48, DPI(240) + 24 - 16), vec2(48, 18), 1.0f, 0xFFD8377C, 0);
            UI::textOut(vec2(UI::width / 2 - 48, DPI(240) + 24), STR_YES, UI::aCenter, 48);
            UI::textOut(vec2(UI::width / 2, DPI(240) + 24), STR_NO, UI::aCenter, 48);
            return;
        }

        if (page == PAGE_LEVEL_STATS) {
            showLevelStats(vec2(0, UI::height / 2 - 75));
            return;
        }

        if (!game->getLevel()->isTitle()) 
            UI::textOut(vec2(eye, DPI(32)), pageTitle[page], UI::aCenter, UI::width);

        if (canFlipPage(-1)) {
            UI::textOut(vec2(DPI(16), DPI(32)), "[", UI::aLeft, UI::width);
            UI::textOut(vec2( 0, DPI(32)), "[", UI::aRight, UI::width - 20);
        }

        if (canFlipPage(1)) {
            UI::textOut(vec2(DPI(16), UI::height - DPI(16)), "]", UI::aLeft, UI::width);
            UI::textOut(vec2( 0, UI::height - DPI(16)), "]", UI::aRight, UI::width - 20);
        }
    ...
    // inventory controls help
        if (page == targetPage && Input::touchTimerVis <= 0.0f) {
            ...
            UI::textOut(vec2(eye + dx, UI::height - DPI(64)), buf, UI::aLeft, UI::width);
            if (chosen) {
                sprintf(buf, STR[STR_HELP_BACK], bBack);
                UI::textOut(vec2(eye, UI::height - DPI(64)), buf, UI::aRight, UI::width - dx);
            }
        }
